...
User Function INSTSXDB()
	...
	Private Titulo   := "InstalaÁ„o de Pacote SYNCSXDB"
	Private aSays    := {}
	Private aButtons := {}
	Private oText	 := Nil
	...
	aAdd(aSays," Rotina para instalaÁ„o de dicion·rios personalizados.")
	aAdd(aSays,"Empresa: " + SM0->M0_CODIGO + "-" + AllTrim(SM0->M0_NOME))
	aAdd(aSays," ATUALIZA«√O DE DICION¡RIOS E TABELAS")
	aAdd(aSays," Esta rotina tem como funÁ„o fazer  a atualizaÁ„o  dos dicion·rios do Sistema ( SX?/SIX )")
	aAdd(aSays," alÈm do conteudo destas tabelas")
...
Rotina de processamento para instalaÁ„o da Consulta Personalizada.
...
Local k        := 0
	...
	If !isBlind()
		Alert("InstalaÁ„o finalizada com sucesso !!")
	Else
		Conout("InstalaÁ„o finalizada com sucesso !!")
	EndIf
...
If (! MSFile(cFileX2, , "TOPCONN") .Or. ! MSFile(cFileX3, , "TOPCONN")) .Or.;
	...
	While ! SX3->(Eof())
		...
		If nRegs > 200 .Or. SX3->(Eof())
			...
			cError := TcSqlError()
			IF ! Empty(cError)
				GrLog("AtenÁ„o. Erro ao importar [" + cFile + "]. Erro: " + cError)
			EndIf
...
While ! QRY->(Eof())
	...
	//-- Campos n„o localizados
	If ! SX3->(DbSeek(QRY->X2_CHAVE))
		GrLog("AtenÁ„o. A lista de campos no SX3 n„o foi definida para tabela [" + QRY->X2_CHAVE + "] !")
	...
	//-- Indices n„o localizados
	If ! SIX->(DbSeek(QRY->X2_CHAVE))
		GrLog("AtenÁ„o. A lista de indices no SIX n„o foi definida para tabela [" + QRY->X2_CHAVE + "] !")
